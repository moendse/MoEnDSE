...
from config import case_name, N_SAMPLES_INIT, N_SAMPLES_ALL
...
from simulation_metrics import evaluate_ADRS, CPI_metric, power_metric, get_orthogonal_array, get_init_by_kmeans, get_init_by_distance_cluster, \
    evaluate_non_uniformity, distance_f, get_di
from config import plot_pareto, N_SPACE_SIZE
...
class OptimizerMultiobj(object):
    ...
    def __init__(self, dimensions, base_estimator="gp", base_estimator2="gp",
        ...
        if self._initial_point_generator is not None:
            ...
            if "orthogonal" == initial_point_generator:
                ...
                self._initial_samples, _, _, _ = get_init_by_kmeans(n_clusters=N_SAMPLES_INIT, random_state=self.rng.randint(0, np.iinfo(np.int32).max))
            elif 'distance' == initial_point_generator:
                self._initial_samples = get_init_by_distance_cluster(n_clusters=N_SAMPLES_INIT)
            else:
    ...
    def get_hvi(self, X_origin, est, est2, stuck_retry=0):
        ...
        if self.surrogate_model_dict['ucb']:
            ...
            if 'Bag' in self.surrogate_model_dict['tag']:
                all_predict, staged_predict = est.predict(X_transform, return_stage=True)
                #print(staged_predict)
                values = np.asarray(staged_predict)
                all_predict2, staged_predict2 = est2.predict(X_transform, return_stage=True)
                values2 = np.asarray(staged_predict2)
    ...
    def get_all_design_point(self):
        ...
        if 36864 == N_SPACE_SIZE:
            var_list = [[] for i in range(10)]
        elif 9216 == N_SPACE_SIZE:
            var_list = [[] for i in range(9)]            
        else:
        ...
        if 2304 == N_SPACE_SIZE:
            design_space_product = itertools.product(var_list[0], var_list[1], var_list[2], var_list[3], var_list[4], var_list[5], var_list[6], var_list[7])
        elif 9216 == N_SPACE_SIZE:
            design_space_product = itertools.product(var_list[0], var_list[1], var_list[2], var_list[3], var_list[4], var_list[5], var_list[6], var_list[7], var_list[8])                  
        else:
            design_space_product = itertools.product(var_list[0], var_list[1], var_list[2], var_list[3], var_list[4], var_list[5], var_list[6], var_list[7], var_list[8], var_list[9])        
    ...
    def _tell(self, x, y, y2, fit=True):
        ...
        if (fit and self._n_initial_points <= 0 and self.surrogate_model_dict['model_is_iterative'] and
            ...
            est2 = self.base_estimator2_

            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                #Xi_transform = self.space.transform(self.Xi)
                Xi_transform = self.Xi
                est.fit(Xi_transform, self.yi)
                est2.fit(Xi_transform, self.yi2)
                # if False:
                #    print(
            ...
            for cand_acq_func in self.cand_acq_funcs_:
                ...
                if self.acq_optimizer == "full":
                    ...
                    if acq_func == "cv_ranking":
                        ...
                        next_x_index, next_x_origin = self.get_hvi(X_origin, est, est2)
                        #next_x_origin = X_origin[next_x_index]
                    ...
                    metric_ADRS, coverage_percent = evaluate_ADRS(real_pareto_optimal_sets=self.real_pareto_data[2],
                                                                  ...
                                                                  coverage=True)
                    #print(f"current_pareto_x metric_ADRS={metric_ADRS}")
                    time_used = datetime.strptime(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "%Y-%m-%d %H:%M:%S") - startTime
